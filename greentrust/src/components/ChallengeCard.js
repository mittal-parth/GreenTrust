...
export default function ChallengeCard({ challenge, type, auth }) {
  ...
  return (
    <div>
      <div
        className={
          "  relative flex-none bg-white border-l-4 rounded-lg shadow-lg mr-6 p-4 w-full " +
      ...
      >
        ...
        <div className="flex flex-col justify-evenly py-4 px-10">
          ...
          <div className="flex flex-col justify-between">
            ...
            {type == 1 && (
              <button
                className="bg-blue bg-darkPrimary text-white font-comfortaa w-fit font-bold py-2 px-4 rounded"
                onClick={async () => {
                  await handleOnClick(1);
                }}
              >
                Accept
              </button>
            ...
            {type == 0 && (
              <div className="flex space-x-2">
                <button
                  className="bg-darkPrimary text-white font-comfortaa font-bold py-2 px-4 w-fit rounded"
                  onClick={async () => await handleOnClick(2)}
                >
                  Approve
                </button>
                <button
                  className="bg-red text-white font-comfortaa font-bold py-2 px-6 w-fit rounded"
                  onClick={async () => await handleOnClick(3)}
                >
                  Reject
                </button>
              </div>
