  ...
  async function getCropDetails() {
    ...
    try {
      ...
      for (let stake of data.stakes) {
        res = await contractCall(auth, "addressToFarmerIds", [
          stake.stakeholder,
        ]);
      ...
      if (res.data == "farmer") {
        ...
        if (
          parseInt(data.farm.farmerId._hex) == parseInt(farmerIdRes.data._hex)
        ) {
          setHasAccess(true);
        }
      }
          ...
          <div className="">
            ...
            <div className="flex mb-10">
              ...
              <div className="grow">
                  ...
                  <div className="flex justify-between items-start xl:items-center pt-4 flex-col xl:flex-row">
                    <div className="flex justify-center gap-4 flex-col max-w-[320px]">
                      <div className="flex flex-row justify-between">
                        <h2 className="mb-0">{data.crop.name}</h2>
                        <Modal
                          anchor={
                            <FontAwesomeIcon
                              icon={faQrcode}
                          ...
                          popover={
                            <QRCard
                              value={`${HOST}/farm/${farmId}/crop/${cropId}`}
                            />
                          }
                        />
                      ...
                      <div className="flex flex-row gap-10">
                        <Info
                          icon={faLocationDot}
                          text={data.farm.location}
                          style="text-red"
                        />
                        <Info
                          icon={faChartPie}
                          text={`${data.crop.size} Acre`}
                          style="text-gray"
                        />
                      </div>
                    </div>
                ...
                <div className="my-20">
                  <FarmerCard profile={data.farmerProfile} />
                </div>
                <div className="flex flex-row gap-10 items-center mb-2">
                  <h3 className="mb-0">Sensors</h3>

                  {hasAccess && (
                    <Link href={`/farm/${farmId}/crop/${cropId}/sensor/add`}>
                      <IconButton icon={faPlus} styles="!w-6 !h-6" />
                    </Link>
                  )}
                </div>
                <div className="grid grid-cols-1: sm:grid-cols-2 gap-10">
                  {data.sensors.length > 0 ? (
                    data.sensors.map((sensor) => (
                      <SensorCard onClick={() => {setHidden(false)}} details={sensor} />
                        
                    ))
                  ) : (
                    <p className="text-gray text-center max-w-[200px]">
                      No sensor added yet!
                    </p>
                  )}
                </div>
                <h3 className="mt-10 mb-0">Stakeholders</h3>
                <div className="flex mt-2 items-center">
                  ...
                  <p className="">
                    <span className="text-primary font-semibold">
                      {parseInt(data.crop.stakeAmount._hex)}/-
                    </span>{" "}
                    each
                  </p>
                </div>
                    ...
                    {!hasAccess ? (
                      <Button
                        ...
                        onClick={async () => {
                          ...
                          try {
                            await contractCall(auth, "addStake", [
                              cropId,
                              { value: parseInt(data.crop.stakeAmount._hex) },
                            ]);
                          } catch (err) {
                            setSnackbarInfo({
                              ...snackbarInfo,
