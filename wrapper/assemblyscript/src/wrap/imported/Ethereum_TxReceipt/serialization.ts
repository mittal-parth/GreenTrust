...
export function writeEthereum_TxReceipt(writer: Write, type: Ethereum_TxReceipt): void {
  ...
  writer.writeOptionalString(type.root);
  writer.context().pop();
  ...
  writer.writeUInt32(type._type);
  writer.context().pop();
  writer.context().push("status", "Box<u32> | null", "writing property");
  writer.writeString("status");
  writer.writeOptionalUInt32(type.status);
  writer.context().pop();
}
...
export function readEthereum_TxReceipt(reader: Read): Ethereum_TxReceipt {
  ...
  while (numFields > 0) {
    ...
    if (field == "to") {
      ...
      _root = reader.readOptionalString();
      reader.context().pop();
    }
      ...
      _status = reader.readOptionalUInt32();
      reader.context().pop();
    }
    reader.context().pop();
  }
